...
- ✓ **Phase 3.5** — Profile modal enrichment (`CreatorProfileModalComponent` opens on card "ⓘ"):
  - ✓ **YouTube section**: `YoutubeCreatorService` wraps the (now flat-shape) `youtube-creator-data` edge fn. Renders live subs / avg views / sponsor freq / engagement rate / avg days between uploads / last upload (relative) / recent videos with **PAID PROMO** pill on flagged titles. No client-side cache table — edge fn hits YouTube API on demand; in-tab cache by creator id avoids same-session re-fetch.
...
- ⏳ **Cleanup**:
  ...
  - `creator_youtube_cache` table is unused by the current edge fn (prod's rewrite dropped the persistence layer). Leave or drop in a future cleanup migration.
