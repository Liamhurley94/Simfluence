...
- **Sibling repo** at `../simfluence-backend` holds Supabase migrations, edge functions, env-switching scripts, and sync tooling. Read its `AGENTS.md` for the source-of-truth workflow.
...
When the cutover finally happens — `develop` is promoted to `main`, legacy HTML retired, current staging Supabase becomes the new prod — **the "Migration context" section above must be updated or removed**, along with the matching reminder in `../simfluence-backend/AGENTS.md`. The "track owner's manual edits" model goes away; this codebase becomes the sole source of truth. Several entries under "Next up" below (cutover prep, sync scripts as obsolete) also need refreshing.

If you're a future contributor or coding-agent session reading this and the cutover has already happened, flag this section and offer to revise.
...
- ✓ **Phase 3.5** — Profile modal enrichment (`CreatorProfileModalComponent` opens on card "ⓘ"):
  - ✓ **YouTube section**: `YoutubeCreatorService` reads from `youtube_creators` table via PostgREST (no on-demand YouTube API calls — all data is nightly-refreshed by the `refresh-youtube-cache` edge fn via pg_cron). Renders subs / avg views / sponsor freq / engagement rate / avg days between uploads / last upload (relative) / recent videos with **PAID PROMO** pill on flagged titles. Header shows "Updated Nh ago" from `stats_refreshed_at`. Legacy `youtube-creator-data` edge fn is deprecated (TODO: delete).
  - ✓ **Twitch section**: `TwitchLiveService.fetchEnrichment(creator)` hits `twitch-live-status` with `with_activity: true`. Renders live/offline state + viewer count + game + the **inactivity badge** (`> 90 days` red / `30–90` amber / `≤ 30` green). Silent-fail to "Twitch live info unavailable — API not configured" when keys aren't set. The earlier Live Channels sidebar feed was retired to mirror prod.
...
- ⏳ **Phase 9** — Account upgrades + Enterprise + Admin UI. Migration `20260515120000_enterprises_and_admin.sql` adds `enterprises` and `enterprise_invites` tables plus `is_admin` + `enterprise_id` columns on `profiles`. Seven new edge functions (`upgrade-to-full`, `apply-for-enterprise`, `admin-list-enterprises`, `admin-approve-enterprise`, `admin-reject-enterprise`, `enterprise-invite-user`, `accept-enterprise-invite`). `AccountComponent` provides settings + upgrade CTAs (Full / Apply for Enterprise) + enterprise-owner invite UI. `AdminComponent` is a four-tab panel hidden behind `adminGuard` (RLS-enforced via `current_user_is_admin()` SECURITY DEFINER helper): Enterprises (approval queue), Add creators (2026-07-11 — YouTube discovery search / review queue / background sweeps / manual add form; see `../simfluence-backend/ARCHITECTURE.md` §12 "Admin creator discovery"), Creators (added-creator status + offline lists + per-row "Add platform" linking), and Usage (YouTube/Twitch API quota). Signup flow accepts `?invite_token=` to redeem an invite. Legacy `validate-password` edge fn retired. See `../simfluence-backend/ARCHITECTURE.md` for the full account/admin/enterprise model.
...
- ✓ **Creator data sync from `main`'s app.html** — `scripts/sync-creators-from-source.ts` reads `git show origin/main:app.html`, extracts the `CREATORS` JS literal, transforms camelCase→snake_case + CSV→array, upserts on `id` (non-destructive) to staging. Staging at 5,746 creators after MENA/UAE Warriors port from main.
- ✓ **Edge functions synced from prod** — `supabase functions download --project-ref <prod>` pulled the source-of-truth versions of all four prod edge functions (`youtube-creator-data`, `validate-password`, `score-creator`, `run-simulation`). Re-applied local `apikey` CORS patches needed by the Angular authInterceptor. `twitch-live-status` doesn't exist on prod — staging is now its source of truth. Source-truth flow going forward: owner edits prod fn in dashboard → we re-pull → diff → re-deploy to staging.
- ✓ **Discovery sort wiring fix** — `FilterPanelComponent` output renamed `change` → `queryChange` to avoid colliding with the native DOM `change` event on its inner `<select>` elements (the parent was receiving the bubbled native event instead of the typed `DiscoveryQuery`).
...
- ✓ **Discovery platform-filtered redesign** — done. Discovery now filters by one platform at a time via a single dropdown (YouTube / Twitch / … / "All platforms" CPI-only view) and reads live stats from the per-platform tables (`youtube_creators`/`twitch_creators`), with an "Updated Xh ago" freshness badge. The stale app.html `subs`/`avg_views` are no longer surfaced.
- ✓ **III.E.4h visual remediation** — done. Derived metrics (CPI / GFI / Estimated Budget Range / Category Benchmarking) are now source-labelled and separated from "YouTube Data", with an always-visible disclaimer footer on every in-app page and CPI relabelled so it never reads as a YouTube metric. Shipped 2026-06-30 to `develop`. Remaining action is external: re-submit Discovery + profile-modal screenshots to YouTube's compliance team.
- ✓ **`sponsor_freq_pct` regex tuning** — done. Promo detection now matches a curated list of sponsorship phrases (`PROMO_PATTERNS` in `refresh-youtube-cache` — e.g. "paid promotion", "sponsored by", "#ad", "use code …", "link in description") instead of a bare `https?://` URL match, so the ~100% false-positive rate is gone.
- ⏳ **Delete legacy `youtube-creator-data` edge fn** — deprecated; replaced by `refresh-youtube-cache` (nightly cron) + `youtube_creators` (PostgREST reads). Delete from staging (`supabase functions delete`) + remove from repo once frontend verification confirms no callers remain.
- ⏳ **Cleanup**:
