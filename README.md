...
- ⏳ **III.E.4h visual remediation** — YouTube ToS violation: derived metrics (CPI Score, GFI Score, Estimated Budget Range, Category Benchmarking) must not be presented as YouTube data. Requires relabelling/separating these from the "YouTube Data" section in the profile modal and discovery cards.
- ⏳ **`sponsor_freq_pct` regex tuning** — the last promo-detection regex (`https?://...`) matches any URL in video descriptions, causing ~100% false-positive rate on `sponsor_freq_pct`. Needs tightening or removal.
